...
def run_job():
    producer = KafkaProducer(bootstrap_servers='35.184.229.234:9092')  # use your VM's external IP Here!
    ...
    t1 = threading.Thread(target=produce_from_file,
                          args=(producer, 'C:\\teaching\DE2025\lab9\data\game\game_data_split1.json'))
    t2 = threading.Thread(target=produce_from_file,
                          args=(producer, 'C:\\teaching\DE2025\lab9\data\game\game_data_split2.json'))
    t1.start()
    t2.start()
